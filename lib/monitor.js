...
export default class Monitor {
  ...
  updateMeta (clid = false) {
    ...
    if (clid) {
      this.teamspeak.findByClid(clid).then(
        client => this.redis.hset(`id:${ client.client_unique_identifier }`, `nickname${ clid }`, client.client_nickname)
      )
    } else {
      this.teamspeak.getOnlineClients().then(clients => {
        let commands = _.map(
          ...
          client => [
            'hmset',
            `id:${ client.client_unique_identifier }`,
  ...
  onClientLeave (client) {
    ...
    this.teamspeak.send('clientgetids', { cluid }).catch(err => {
      if (err.id === 1281) this.redis.srem('online', cluid)
