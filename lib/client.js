...
export class Client extends EventEmitter {
  static get STATUS_NOCON() { return 0 }
  static get STATUS_ERROR() { return 1 }
  static get STATUS_READY() { return 2 }
  static get STATUS_TIMEOUT() { return 3 }
  static get RELAYED_EVENTS() { return ['channeledited'] }
  ...
  _mountEvents () {
    ...
    this.connection.on('textmessage', data => {
      ...
      let handled = this.emit('textmessage', data)
  ...
  connect (config) {
    ...
    this.connection.on('connect', () => {
      Promise.join(
        ...
        this.send('whoami', {}, true).then(reply => this.ownClid = reply.client_id)
      )
  ...
  setClientChannelGroupByDbId (dbid, channelId, channelGroupId = this.config.defautlChannelGroupId){
    return this.send('setclientchannelgroup', { cgid: channelGroupId, cid: channelId, cldbid: dbid})
  }

  listChannelGroupAssignments(filter = {}) {
    return this.send('channelgroupclientlist', filter)
  }
  ...
  getChannelGroupsByCluid(cluid) {
    return this.getDbIdFromCluid(cluid).then(dbid => this.listChannelGroupAssignments({ cldbid: dbid }))
  }

  setClientChannelGroupByCluid (cluid, channelId, channelGroupId) {
    return this.getDbIdFromCluid(cluid).then(dbid => {
      this.setClientChannelGroupByDbId(dbid, channelId, channelGroupId)
    })
  }

  removeAllClientChannelGroupsByCluid (cluid) {
    return this.getChannelGroupsByCluid(cluid).then(channelGroups => {
      if (_.isArray(channelGroups)) {
        return Promise.all(_.map(channelGroups, channelGroup => this.setClientChannelGroupByDbId(channelGroup.cldbid, channelGroup.cid)))
      }
      return this.setClientChannelGroupByDbId(channelGroups.cldbid, channelGroups.cid)
    })
  }
}
