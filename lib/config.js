  ...
  _.forOwn(collection, (value, key) => {
    ...
    if (_.isNumber(value) || _.isString(value) || _.isBoolean(value) || value == null) {
      if (envVal && _.isString(envVal)) {
        ...
        else if (/^\s*\d+\s*$/i.test(envVal)) envVal = _.parseInt(envVal.trim())
...
export default (prefix, dir = cwd()) => {
  try {
    ...
    throw new Error(`Default config not found in ${dir}`)
  }
  try {
    var localConfig = require(join(dir, 'config.local.json'))
  } catch (e) {
    var localConfig = ''
  }
